    ...
    def forward(self, x):
        if isinstance(x,list):
            ...
            for i in range(len(x)):
                ...
                xz = x[i] #(B,L,C)
                B = len(xz)
                xz = xz.view(B,H,W,-1).permute(0, 3, 1, 2)
                xz = self.conv1(xz)
                xz = self.act(xz)
                xz = self.conv2(xz)
                xz = self.act(xz)
...
class TinyViTBlock(nn.Module):
    ...
    def forward(self, image_lists,res_out): #(B,HW,C)
        H_x, W_x = self.input_resolution_x
        H_z, W_z = self.input_resolution_z

        if self.num_depth == self.total_depth - 1:  # 最后一层做x,z交互
            # asf拆分做global att x(B,L,C)
            for i in range(len(image_lists)):
                if i == 0:
                    xz = image_lists[i]
                else:
                    xz = torch.cat((xz,image_lists[i]),dim=1)
            # xz(B/n,L*n,C)
            ...
            for i in range(len(image_lists)):
                ...
                image_lists[i] = xm[:, H_x*W_x*i:H_x*W_x*(i+1), :]
            ...
            for i in range(len(image_lists)):
                ...
                res_x = x
                if self.window_size == H and W == self.window_size:
                    x = self.attn(x)
                else:
                    x = x.view(B, H, W, C)
...
class TinyViT(nn.Module):
    ...
    def forward_features(self, image_lists):
        ...
        for i in range(start_i, len(self.layers)):
            layer = self.layers[i]
            out,res_out = layer(out,res_out) #out==image_list,res_out==list
        # out[i] == [B,L,C]
        cls = out[0].mean(1).unsqueeze(1)#[bs,1,576]
