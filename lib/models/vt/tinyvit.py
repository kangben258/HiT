...
class PatchMerging(nn.Module):
    def __init__(self, input_resolution_x,input_resolution_z, dim, out_dim, activation):
        super().__init__()
    ...
    def forward(self, x):
        if x.ndim == 3: #(B,L,C)
            ...
            B = len(x)
            # (B, C, H, W)
            x = x.view(B, H_x, W_x, -1).permute(0, 3, 1, 2)
    ...
    def __init__(self, dim, input_resolution_x,input_resolution_z, depth,
        ...
        if downsample is not None:
            self.downsample = downsample(
                input_resolution_x,input_resolution_z, dim=dim, out_dim=out_dim, activation=activation)
        else:
...
class TinyViTBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution_x,input_resolution_z, num_heads, window_size=7,
        ...
        self.attn_g = Attention(dim, head_dim, num_heads,
    ...
    def forward(self, x): #(B,HW,C)
        ...
        H_z, W_z = self.input_resolution_z
        B, L, C = x.shape
        assert L == H_x * W_x == H_z * W_z, "input feature has wrong size"
        res_x = x
        if self.num_depth == self.total_depth - 1: #最后一层做x,z交互
            # asf拆分做global att
            n = self.template_number + 1
            for i in range(n):
                if i == 0:
                    xz = x[:int(B/n),:,:]
                else:
                    xz = torch.cat((xz,x[int(B/n*i):int(B/n*(i+1)),:,:]),dim=1)
            # xz(B/n,L*n,C)
            xm = self.attn_g(xz)#(b/N,L*N,C)
            ...
            for i in range(n):
                if i == 0:
                    ...
                    xs = torch.cat((xs,xm[:,L*i:L*(i+1),:]),dim=0)
            x = xs#(B,L,C)
        else:
            if self.window_size == H_x == H_z and W_z == W_x == self.window_size:
                x = self.attn(x)
            else:
                x = x.view(B, H_x, W_x, C)
                pad_b = (self.window_size - H_x %
                         self.window_size) % self.window_size
                pad_r = (self.window_size - W_x %
                         self.window_size) % self.window_size
                ...
                if padding:
                    x = x[:, :H_x, :W_x].contiguous()

                x = x.view(B, L, C)

            x = res_x + self.drop_path(x)

            x = x.transpose(1, 2).reshape(B, C, H_x, W_x)
            x = self.local_conv(x)
...
class TinyViT(nn.Module):
    def __init__(self, search_size=256,template_size=256,
    ...
    def forward_features(self, image_lists):
        ...
        for i in range(len(image_lists)):
            ...
            if i == 0:
                ...
                xz = torch.cat((xz,x),dim=0)

        x = self.patch_embed(xz) #(N,C,H,W)
        #x = self.layers[0](x)   #(N,HW,C)
        start_i = 0
...
@register_tiny_vit_model
def tiny_vit_21m_224(pretrained=False, num_classes=1000, drop_path_rate=0.2,search_size=256,template_size=256):
    return TinyViT(
