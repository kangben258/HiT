...
class NECK_FPN_TINYVIT(nn.Module):
...
class NECK_MID_TINYVIT(nn.Module):
...
class NECK_MIN_TINYVIT(nn.Module):
...
def build_neck(cfg, backbone_channels, num_x, backbone_embed_dim_list):
    if "tiny_vit" in cfg.MODEL.BACKBONE.TYPE:
        if cfg.MODEL.NECK.TYPE == "FPN":
            neck = NECK_FPN_TINYVIT(
        ...
        elif cfg.MODEL.NECK.TYPE == "MINF":
            neck = NECK_MIN_TINYVIT(
                num_x, backbone_channels, backbone_channels,
                cfg.MODEL.HIDDEN_DIM,
                cfg.MODEL.NECK.STRIDE,
                cfg.MODEL.NECK.NUM_LAYERS,
