...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                elif self.frame_sample_mode == "temporal":
                    template_frame_ids, search_frame_ids = self.get_frame_ids_temporal(visible)
                ...
                elif self.frame_sample_mode == "trident_order":
                    template_frame_ids, search_frame_ids = self.get_frame_ids_trident_order(visible)
        ...
        # self.show(data, 'template_images', 0)
        # self.show(data, 'search_images', 0)
        return data

    # def show(self, data, str, i):
    #     image = data[str][i]
    #     import cv2
    #     cv2.imshow(str, image.permute(1,2,0).numpy())
    ...
    def get_frame_ids_trident_order(self, visible):
        # get template and search ids in a 'trident' manner
        template_frame_ids_extra = []
        while None in template_frame_ids_extra or len(template_frame_ids_extra) == 0:
            template_frame_ids_extra = []
        ...
        template_frame_ids = template_frame_id1 + template_frame_ids_extra
        if search_frame_ids < template_frame_id1:
            template_frame_ids.sort(reverse=True)
        else:
            template_frame_ids.sort(reverse=False)
        return template_frame_ids, search_frame_ids

    def get_frame_ids_temporal(self, visible):
        # get template and search ids in a 'temporal' manner
        search_frame_ids = []
        gap_increase = 0
        while None in search_frame_ids or len(search_frame_ids) == 0:
            # first randomly sample two frames from a video
            template_frame_ids = self._sample_visible_ids(visible, num_ids=1)  # the initial template id
            search_frame_ids = self._sample_visible_ids(visible, num_ids=1,
                                                        min_id=template_frame_ids[0]-self.max_gap-gap_increase,
                                                        max_id=template_frame_ids[0]+self.max_gap+gap_increase)  # the search region id
            if search_frame_ids is None:
                gap_increase += 5
                continue
            # get the dynamic template id
            for i in range(self.num_search_frames-1):
                if template_frame_ids[0] > search_frame_ids[i]:
                    min_id, max_id = search_frame_ids[i] - self.max_gap-gap_increase, search_frame_ids[i]
                else:
                    min_id, max_id = search_frame_ids[i], search_frame_ids[i] + self.max_gap+gap_increase
                f_id = self._sample_visible_ids(visible, num_ids=1, min_id=min_id, max_id=max_id)
                if f_id is None:
                    search_frame_ids += [None]
                    break
                else:
                    search_frame_ids += f_id
            gap_increase += 5
            if gap_increase > 1000:
                print("too large gap")
                print(str(gap_increase))
...
class NormalSampler(torch.utils.data.Dataset):
    """ Class responsible for sampling frames from training sequences to form batches.
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            try:
                ...
                search_frames, _, _ = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict)
                ...
                valid = False
            # if data['dataset'] == 'got10k':
            # self.show(data, 'template_images', 1)
            # self.show(data, 'template_images', 0)
            # self.show(data, 'search_images', 0)
        return data
    ...
    #     image = data[strr][i]
    #     import cv2
    #     cv2.imshow(strr+str(i), image.permute(1,2,0).numpy())
    #     if cv2.waitKey() & 0xFF == ord('q'):
    #         pass
...
class MAESampler(torch.utils.data.Dataset):
    """ Class responsible for sampling frames from training sequences to form batches.
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 num_search_frames, processing=no_processing,
                 frame_sample_mode='mae'):
        """
        args:
            datasets - List of datasets to be used for training
            p_datasets - List containing the probabilities by which each dataset will be sampled
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset):
        ...
        while not enough_visible_frames:
            ...
            enough_visible_frames = visible.type(torch.int64).sum().item() > 2 * (
                    self.num_search_frames) and len(visible) >= 20
            enough_visible_frames = enough_visible_frames or not is_video_dataset
        return seq_id, visible, seq_info_dict


class UBBRSampler(torch.utils.data.Dataset):
    """ Class responsible for sampling frames from training sequences to form batches.
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'single':
                    # Sample test and train frames in a causal manner, i.e. search_frame_ids > template_frame_ids
                    while search_frame_ids is None:
                        search_frame_ids = self._sample_visible_ids(visible, num_ids=1, allow_invisible=True)
                else:
            ...
            try:
                ...
                # # anno: [x1, y1, w, h]
                # x1 = torch.randint(0, W, (1,), dtype=float)
                # x2 = torch.randint(0, W, (1,), dtype=float)
                # y1 = torch.randint(0, H, (1,), dtype=float)
                # y2 = torch.randint(0, H, (1,), dtype=float)
                # x_min = min(x1, x2)
                # y_min = min(y1, y2)
                # w = abs(x1 - x2)
                # h = abs(y1 - y2)
                ...
                # anno: [x1, y1, w, h]
                x_center = torch.randint(0, W, (1,), dtype=float)
                y_center = torch.randint(0, H, (1,), dtype=float)
                w = torch.randint(0, W // 2, (1,), dtype=float)
                h = torch.randint(0, H // 2, (1,), dtype=float)
                x_min = max(x_center - w/2, torch.zeros((1,),dtype=float))
                y_min = max(y_center - h/2, torch.zeros((1,),dtype=float))
                x_max = min(x_center + w/2, torch.tensor(W, dtype=torch.float).reshape([1,]))
                y_max = min(y_center + h/2, torch.tensor(H, dtype=torch.float).reshape([1,]))
                fake_anno = [torch.cat((x_min, y_min, x_max-x_min, y_max-y_min))]

                # fake_anno = data['search_anno']

                data = TensorDict({'template_images': search_frames,
                                   'template_anno': fake_anno,
                                   'template_masks': search_masks,
                                   'search_images': search_frames,
                                   'search_anno': fake_anno,
                                   'search_masks': search_masks,
                                   'dataset': dataset.get_name()})
                # make data augmentation
                data = self.processing(data)
