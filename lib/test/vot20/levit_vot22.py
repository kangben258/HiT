...
class levit_vot22(object):
    def __init__(self, tracker_name='vt', para_name='lv_fpn_256_c384_3sch'):
        # create tracker
    ...
    def track(self, img_rgb):
        ...
        final_bbox = vot.Rectangle(pred_bbox[0],pred_bbox[1],pred_bbox[2],pred_bbox[3])
...
def run_vot_exp(tracker_name, para_name, vis=False):
    ...
    save_root = os.path.join('/home/kb/EdgeTrack-main/', para_name)
    if vis and (not os.path.exists(save_root)):
        os.mkdir(save_root)
    tracker = levit_vot22(tracker_name=tracker_name, para_name=para_name)
    handle = vot.VOT("mask")
