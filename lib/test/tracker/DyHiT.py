...
import numpy as np
import time
class DyHiT(BaseTracker):
    def __init__(self, params, dataset_name):
        super(DyHiT, self).__init__(params)
        network = build_dyhit(params.cfg)
        network.load_state_dict(torch.load(self.params.checkpoint, map_location='cpu')['net'], strict=True)
        self.cfg = params.cfg
        utils.replace_batchnorm(network.model1.backbone.body)
        ...
        self.frame_id = 0
        if self.debug == 2:
            save_dir = "/home/kb/kb/vis_results"
        ...
        self.val_inter = params.interval
        self.score_t = self.cfg.TEST.SCORE_T
    ...
    def track(self, image, info: dict = None):
        ...
        if self.frame_id % self.val_inter == 0 or self.frame_id==1:
            val_status = True
        else:
            val_status = False
        with torch.no_grad():
            start = time.time()
            out_dict,score = self.network(images_list=images_list,first_score=self.first_score,threshold=self.threshold,frame=val_status,score_t=self.score_t)
            end = time.time()
        ...
        if self.debug == 2:
            ...
            cv2.rectangle(image_BGR, (int(x1_1),int(y1_1)), (int(x1_1+w_1),int(y1_1+h_1)), color=(0,0,255), thickness=2)
            cv2.putText(image_BGR, 'Score:' + str(score), (100, 200), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2,
                        cv2.LINE_AA)
