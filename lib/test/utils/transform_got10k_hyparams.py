...
def transform_got10k(tracker_name, cfg_name):
    env = env_settings()
    result_dir = os.path.join(env.results_path, "%s/%s/" % (tracker_name, cfg_name))
    names = os.listdir(result_dir)
    for name in names:
        if "got10k0" in name:
            dest_name = name+'_submit'
            src_dir = os.path.join(result_dir, name)
            dest_dir = os.path.join(result_dir, dest_name)
