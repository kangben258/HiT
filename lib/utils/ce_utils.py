...
def generate_mask_cond(cfg, bs, device, gt_bbox):
    ...
    if cfg.MODEL1.BACKBONE.CE_TEMPLATE_RANGE == 'ALL':
        box_mask_z = None
    elif cfg.MODEL1.BACKBONE.CE_TEMPLATE_RANGE == 'CTR_POINT':
    ...
    elif cfg.MODEL1.BACKBONE.CE_TEMPLATE_RANGE == 'CTR_REC':
    ...
    elif cfg.MODEL1.BACKBONE.CE_TEMPLATE_RANGE == 'GT_BOX':
        ...
        box_mask_z = F.interpolate(box_mask_z, scale_factor=1. / cfg.MODEL1.BACKBONE.STRIDE, mode='bilinear',
                                   align_corners=False)
        box_mask_z = box_mask_z.flatten(1).to(torch.bool)
