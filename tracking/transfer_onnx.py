...
if __name__ == "__main__":
    ...
    # update cfg
    args = parse_args()
    yaml_fname = '/home/kb/EdgeTrack-main/experiments/%s/%s.yaml' % (args.script, args.config)
    config_module = importlib.import_module('lib.config.%s.config' % args.script)
    cfg = config_module.cfg
    config_module.update_config_from_file(yaml_fname)
    save_name = "/home/kb/EdgeTrack-main/checkpoints/train/%s/%s/VT_ep%04d.onnx" % (args.script, args.config, cfg.TEST.EPOCH)
    # build the model
