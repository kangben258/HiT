...
if __name__ == "__main__":
    ...
    model = LeViT_384(fuse=True, pretrained=True)
    model.eval()
    # print(torch_model)
    ...
    macs1, params1 = profile(model, inputs=[image], verbose=False)
    ...
    torch.onnx.export(model,  # model being run
                      (image),  # model input (a tuple for multiple inputs)
    ...
    ort_inputs = {'input': to_numpy(image).astype(np.float32)
                  }
    """warmup (the first one running latency is quite large for the onnx model)"""
    for i in range(10):
        # pytorch inference
        torch_outs = model(image)
        # onnx inference
